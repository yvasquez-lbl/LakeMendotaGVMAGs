...
#saveRDS( all_year, file = "snps_all_file.rds")

all_year_processed <- readRDS(file = "snps_all_file.rds")
...
counts_prop <- counts_df %>%
  mutate(total_selection_genes = positive_gene_count + negative_gene_count,
         prop_positive = positive_gene_count / total_selection_genes)
model_balance <- glmmTMB(
  cbind(positive_gene_count, negative_gene_count) ~ Collection.Year + 
    (1 | family) + (1 | giantvirus),
  family = betabinomial,
  data = counts_df
)
